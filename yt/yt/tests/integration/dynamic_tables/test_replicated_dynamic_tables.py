...
class TestReplicatedDynamicTables(TestReplicatedDynamicTablesBase):
    ...
    def test_reshard_replicated_table_filters_out_of_range_log_rows(self):
        # After a split reshard every new tablet inherits a clone of the originator's
        # replication log chunks, so each new tablet's log contains rows whose keys belong
        # to sibling tablets. If replication is lagging at reshard time, the replicator
        # on each new tablet will read those out-of-range rows from its cursor and must
        # drop them; otherwise every new tablet re-sends rows that semantically belong
        # to exactly one sibling, multiplying replica write traffic by the fan-out.
        self._create_cells()
        self._create_replicated_table(
            "//tmp/t",
        ...
        initial_rows = [
            {"key": k, "value1": "pre", "value2": k}
            for k in range(10)
        ]
        insert_rows("//tmp/t", initial_rows)
        wait(lambda: lookup_rows(
            "//tmp/r",
            [{"key": r["key"]} for r in initial_rows],
            driver=self.replica_driver) == initial_rows)

        alter_table_replica(replica_id, enabled=False)
        # Wait for the replicator fiber to actually stop so no further writes race with us.
        wait(lambda: get("#{}/@state".format(replica_id)) == "disabled")

        lagging_rows = [
            {"key": k, "value1": "post", "value2": k}
            for k in range(10, 20)
        ]
        insert_rows("//tmp/t", lagging_rows)

        sync_flush_table("//tmp/t")
        sync_unmount_table("//tmp/t")

        # Pivot keys are chosen so that the lagging rows (keys 10..19) are spread
        # evenly across four new tablets — each tablet's cloned log contains all 10
        # lagging rows but only 2–3 of them belong to its key range.
        reshard_table("//tmp/t", [[], [5], [10], [15]])
        sync_mount_table("//tmp/t")

        def get_replica_write_count():
            return get("//tmp/r/@tablets/0/performance_counters/dynamic_row_write_count")

        # Sanity: replica received exactly the 10 initial writes before the reshard.
        wait(lambda: get_replica_write_count() == len(initial_rows))

        alter_table_replica(replica_id, enabled=True)

        # Wait until every tablet's replication cursor has advanced past the entire log.
        # This guarantees the replicator has seen every lagging row on every new tablet;
        # any duplicate replication caused by a missing filter must have already happened.
        def all_tablets_caught_up():
            tablet_count = get("//tmp/t/@tablet_count")
            for i in range(tablet_count):
                committed = get("#{}/@tablets/{}/committed_replication_row_index".format(replica_id, i))
                if committed < len(initial_rows) + len(lagging_rows):
                    return False
            return True
        wait(all_tablets_caught_up)

        all_rows = sorted(initial_rows + lagging_rows, key=lambda r: r["key"])
        assert lookup_rows(
            "//tmp/r",
            [{"key": r["key"]} for r in all_rows],
            driver=self.replica_driver) == all_rows

        # After the split reshard there should be exactly one replication write per
        # lagging row. Without the filter fix each of the four new tablets would re-send
        # every lagging row it sees in its cloned log (10 rows × 4 tablets = 40 writes),
        # so the counter would settle at 10 + 40 = 50 instead of 10 + 10 = 20.
        expected_total_writes = len(initial_rows) + len(lagging_rows)
        actual_writes = get_replica_write_count()
        assert actual_writes == expected_total_writes, (
            "replica received {} dynamic writes, expected {} — "
            "the table replicator is not filtering out-of-range log rows"
            .format(actual_writes, expected_total_writes))
